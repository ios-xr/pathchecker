...
sudo pip --proxy $https_proxy install pycparser==2.13 idna 
sudo pip --proxy $https_proxy install cryptography==1.1
sudo pip --proxy $https_proxy install paramiko netmiko
...
cd ansible/ && $SET_PROXY && sudo -E python setup.py install
